...
#define TAG "NuerolynxSettings"

MomentumSettings momentum_settings = {
    .asset_pack = "Nuerolynx", // Nuerolynx theme; saved preferences take precedence
    .anim_speed = 100, // 100%
    ...
    .bar_borders = true, // ON
    .bar_background = true, // ON: animated Nuerolynx trace background
...
void momentum_settings_load(void) {
    ...
    bool migrated = false;
    if(!storage_common_exists(storage, NUEROLYNX_SETTINGS_PATH) &&
       storage_common_exists(storage, LEGACY_MOMENTUM_SETTINGS_PATH)) {
        migrated = storage_common_copy(
                       storage, LEGACY_MOMENTUM_SETTINGS_PATH, NUEROLYNX_SETTINGS_PATH) == FSE_OK;
    }
    FlipperFormat* file = flipper_format_file_alloc(storage);
    if(flipper_format_file_open_existing(file, NUEROLYNX_SETTINGS_PATH)) {
...
#define entry momentum_settings_entries[entry_i]
    ...
    if(strcmp(momentum_settings.asset_pack, "Momentum") == 0) {
        strlcpy(momentum_settings.asset_pack, "Nuerolynx", sizeof(momentum_settings.asset_pack));
        migrated = true;
    }
    ...
    if(migrated) {
        ...
        storage_common_remove(storage, LEGACY_MOMENTUM_SETTINGS_PATH);
        furi_record_close(RECORD_STORAGE);
    }
...
void momentum_settings_save(void) {
    ...
    if(flipper_format_file_open_always(file, NUEROLYNX_SETTINGS_PATH)) {
        int32_t tmp_int;
        uint32_t tmp_uint;
        for(size_t entry_i = 0; entry_i < COUNT_OF(momentum_settings_entries); entry_i++) {
#define entry momentum_settings_entries[entry_i]

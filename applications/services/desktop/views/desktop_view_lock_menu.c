...
void desktop_lock_menu_draw_callback(Canvas* canvas, void* model) {
    ...
    for(size_t i = 0; i < DesktopLockMenuIndexTotalCount; ++i) {
        ...
        case DesktopLockMenuIndexBluetooth:
            ...
            break;
        case DesktopLockMenuIndexNuerolynx:
            icon = &I_CC_Nuerolynx_25x25;
            break;
        ...
        if(toggle) {
            ...
            if(i == DesktopLockMenuIndexNuerolynx) {
                // The 25x25 logo sits one pixel inside the tile. Draw its blank
                // pixels transparently so they cannot erase the rounded frame.

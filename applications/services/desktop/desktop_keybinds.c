...
static void desktop_keybinds_upgrade_firmware_name(FuriString* keybind) {
    if(furi_string_equal(keybind, "Momentum") || furi_string_equal(keybind, "Xtreme")) {
        furi_string_set(keybind, "Nuerolynx");
    }
}
        ...
        if(success) {
            ...
            for(DesktopKeybindType type = 0; type < DesktopKeybindTypeMAX; type++) {
                for(DesktopKeybindKey key = 0; key < DesktopKeybindKeyMAX; key++) {
                    ...
                    desktop_keybinds_upgrade_firmware_name(keybind);
    ...
    if(flipper_format_file_open_existing(file, DESKTOP_KEYBINDS_PATH)) {
        for(DesktopKeybindType type = 0; type < DesktopKeybindTypeMAX; type++) {
            for(DesktopKeybindKey key = 0; key < DesktopKeybindKeyMAX; key++) {
                ...
                desktop_keybinds_upgrade_firmware_name((*keybinds)[type][key]);
            }
        }
    } else {
    fail:
...
void desktop_run_keybind(Desktop* desktop, InputType _type, InputKey _key) {
    ...
    if(furi_string_equal(keybind, "_")) {
        ...
        loader_start_detached_with_gui_error(
            desktop->loader, EXT_PATH("apps/Tools/System/nightstand.fap"), "");

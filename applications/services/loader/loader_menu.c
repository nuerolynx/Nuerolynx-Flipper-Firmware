...
enum {
    ...
    LoaderMenuIndexSmartSearch = (uint32_t)-4,
};
...
static void loader_menu_smart_search_callback(void* context, uint32_t index) {
    UNUSED(index);
    UNUSED(context);
    loader_menu_start(LOADER_APPLICATIONS_SEARCH_NAME);
}
...
static void loader_menu_build_menu(LoaderMenuApp* app, LoaderMenu* menu) {
    menu_add_item(
        app->primary_menu,
        LOADER_APPLICATIONS_SEARCH_NAME,
        &I_search_10px,
        LoaderMenuIndexSmartSearch,
        loader_menu_smart_search_callback,
        NULL);
    menu_add_item(
        app->primary_menu,
        ...
        while(stream_read_line(stream, line)) {
            ...
            if(version == 0) {
                if(furi_string_equal(line, "RFID")) {
                    ...
                    furi_string_set(line, "Nuerolynx");
                }
            }
            if(furi_string_equal(line, "Momentum")) {
                // Upgrade saved main menus from the previous Nuerolynx build.
                furi_string_set(line, "Nuerolynx");
            }

...
static void menu_get_name(MenuItem* item, FuriString* name, bool shorter) {
    ...
    if(shorter) {
        if(!furi_string_cmp(name, "Nuerolynx")) {
            furi_string_set(name, "NLX");
            return;
    ...
    if(items_count) {
        ...
        case MenuStyleList: {
            for(uint8_t i = 0; i < 3; i++) {
                ...
                // A 20 px slot preserves the original position of standard
                // 14 px icons while giving the Nuerolynx mark one clear pixel
                // inside the selected row frame.
                menu_centered_icon(canvas, item, 1, 22 * i, 20, 20);
        ...
        case MenuStyleDsi: {
            for(int8_t i = -2; i <= 2; i++) {
                ...
                menu_centered_icon(canvas, item, pos_x - 10, pos_y - 10, 20, 20);
            }
        ...
        case MenuStyleVertical: {
            ...
            const size_t visible_items = 6;
            const size_t row_height = 21;
            if(shift_position >= position ||
               shift_position + visible_items - 1 <= position) {
                // In case vertical_offset is out of sync due to changing menu styles
                shift_position = CLAMP(
                    MAX((int32_t)position - 3, 0),
                    MAX(
                        (int32_t)MenuItemArray_size(model->items) - (int32_t)visible_items,
                        0),
                    0);
            ...
            for(size_t i = 0; i < visible_items; i++) {
                ...
                bool selected = item_i == position;
                if(selected) {
                    elements_slightly_rounded_box(canvas, 0, y_off, 64, row_height);
                    canvas_set_color(canvas, ColorWhite);
                }
                item = MenuItemArray_get(model->items, item_i);
                menu_centered_icon(canvas, item, 0, y_off, 21, row_height);
                menu_get_name(item, name, true);
                size_t scroll_counter = menu_scroll_counter(model, selected);
                elements_scrollable_text_line(
                    canvas, 22, y_off + 15, 41, name, scroll_counter, false);
                if(selected) {
                    canvas_set_color(canvas, ColorBlack);
                }
            }

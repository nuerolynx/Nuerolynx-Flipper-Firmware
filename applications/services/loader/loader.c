...
static void loader_do_applications_show(Loader* loader, bool search_mode) {
    if(!loader->loader_applications) {
        loader->loader_applications = loader_applications_alloc(
            loader_applications_closed_callback, loader, search_mode);
    }
}
...
static bool loader_resolve_relocated_fap(
    Storage* storage,
    const char* requested_path,
    FuriString* resolved_path) {
    const size_t requested_length = strlen(requested_path);
    if(strncmp(requested_path, EXT_PATH("apps/"), strlen(EXT_PATH("apps/"))) != 0 ||
       requested_length < strlen(".fap") ||
       strcmp(requested_path + requested_length - strlen(".fap"), ".fap") != 0) {
        return false;
    }

    const char* requested_name = strrchr(requested_path, '/');
    if(!requested_name || !requested_name[1]) return false;
    requested_name++;

    bool found = false;
    FileInfo file_info;
    FuriString* candidate_path = furi_string_alloc();
    DirWalk* dir_walk = dir_walk_alloc(storage);

    if(dir_walk_open(dir_walk, EXT_PATH("apps"))) {
        while(dir_walk_read(dir_walk, candidate_path, &file_info) == DirWalkOK) {
            if(file_info_is_dir(&file_info)) continue;

            const char* candidate = furi_string_get_cstr(candidate_path);
            const char* candidate_name = strrchr(candidate, '/');
            if(candidate_name && strcmp(candidate_name + 1, requested_name) == 0) {
                if(found) {
                    FURI_LOG_W(TAG, "Ambiguous relocated FAP: %s", requested_name);
                    found = false;
                    break;
                }
                furi_string_set(resolved_path, candidate_path);
                found = true;
            }
        }
    ...
    furi_string_free(candidate_path);
    return found;
}
...
static LoaderMessageLoaderStatusResult loader_do_start_by_name(
    ...
    FuriString* relocated_fap = furi_string_alloc();
    ...
    do {
        ...
        if(strcmp(name, LOADER_APPLICATIONS_NAME) == 0 ||
           strcmp(name, LOADER_APPLICATIONS_SEARCH_NAME) == 0) {
            loader_do_applications_show(
                loader, strcmp(name, LOADER_APPLICATIONS_SEARCH_NAME) == 0);
            status.value = loader_make_success_status(error_message);
            break;
        }
        ...
        // check Faps
        {
            ...
            if(!storage_file_exists(storage, name) &&
               loader_resolve_relocated_fap(storage, name, relocated_fap)) {
                name = furi_string_get_cstr(relocated_fap);
                FURI_LOG_I(TAG, "Resolved relocated FAP: %s", name);
            }
            if(storage_file_exists(storage, name)) {
    ...
    furi_string_free(relocated_fap);

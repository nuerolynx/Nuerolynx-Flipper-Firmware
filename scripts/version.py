...
class GitVersion:
    ...
    def get_version_info(self):
        ...
        branch = (
            ...
            or tag
            or self._exec_git("rev-parse --abbrev-ref HEAD").removeprefix("nlx-")
            or "unknown"
        )

        nlx_release = os.environ.get("NLX_RELEASE_VERSION", "").strip()
        if tag:
            release = tag
            for prefix in ("Nuerolynx-", "nuerolynx-", "NLX-", "nlx-"):
                if release.startswith(prefix):
                    release = release[len(prefix) :]
                    break
            version = f"Nuerolynx {release}"
        elif nlx_release:
            version = f"Nuerolynx {nlx_release}"
        else:
            version = "Nuerolynx dev"
    ...
    def _get_git_origins(self):
        # Do not expose the upstream repository as the installed firmware's
        # identity. Upstream attribution remains in the source tree.
        return set(["Nuerolynx-Firmware"])

    def _get_upstream_git_origins(self):
        try:

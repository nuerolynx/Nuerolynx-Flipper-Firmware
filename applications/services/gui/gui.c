...
#define NUEROLYNX_STATUSBAR_PACKET_FPS 6U
#define NUEROLYNX_STATUSBAR_PACKET_PERIOD_MS (1000U / NUEROLYNX_STATUSBAR_PACKET_FPS)
#define NUEROLYNX_STATUSBAR_PACKET_PHASES 40U
#define NUEROLYNX_STATUSBAR_PACKET_DOTS 4U
#define NUEROLYNX_STATUSBAR_PACKET_SPACING 2U
#define NUEROLYNX_STATUSBAR_PACKET_GAP 9U

typedef struct {
    uint8_t x;
    uint8_t y;
} NuerolynxStatusbarPacketPoint;

static NuerolynxStatusbarPacketPoint
    gui_nuerolynx_statusbar_packet_point(uint8_t position) {
    NuerolynxStatusbarPacketPoint point;

    /* Clockwise, closed S-shaped path. The upper trace is offset one pixel
     * left and the lower trace one pixel right, with opposing end bends. */
    if(position < 19U) {
        point.x = 31U + position;
        point.y = 5U;
    } else if(position == 19U) {
        point.x = 50U;
        point.y = 6U;
    } else if(position < 39U) {
        point.x = 50U - (position - 20U);
        point.y = 7U;
    } else {
        point.x = 31U;
        point.y = 6U;
...
static void gui_draw_nuerolynx_statusbar_packets(Canvas* canvas) {
    uint32_t frame_ticks =
        furi_kernel_get_tick_frequency() / NUEROLYNX_STATUSBAR_PACKET_FPS;
    if(frame_ticks == 0) frame_ticks = 1;
    const uint8_t phase =
        (furi_get_tick() / frame_ticks) % NUEROLYNX_STATUSBAR_PACKET_PHASES;

    /* Restore the complete ribbon first so it remains connected to the
     * status-card side. A short gap is then moved around the closed path;
     * its moving boundaries carry the four one-pixel packet dots. */
    canvas_set_color(canvas, ColorBlack);
    for(uint8_t position = 0; position < NUEROLYNX_STATUSBAR_PACKET_PHASES; position++) {
        const NuerolynxStatusbarPacketPoint point =
            gui_nuerolynx_statusbar_packet_point(position);
        canvas_draw_dot(canvas, point.x, point.y);
    }

    canvas_set_color(canvas, ColorWhite);
    for(uint8_t offset = 0; offset < NUEROLYNX_STATUSBAR_PACKET_GAP; offset++) {
        const uint8_t position = (phase + offset) % NUEROLYNX_STATUSBAR_PACKET_PHASES;
        const NuerolynxStatusbarPacketPoint point =
            gui_nuerolynx_statusbar_packet_point(position);
        canvas_draw_dot(canvas, point.x, point.y);
    }
    ...
    canvas_set_color(canvas, ColorBlack);
    for(uint8_t dot = 0; dot < NUEROLYNX_STATUSBAR_PACKET_DOTS; dot++) {
        const uint8_t position =
            (phase + 1U + (dot * NUEROLYNX_STATUSBAR_PACKET_SPACING)) %
            NUEROLYNX_STATUSBAR_PACKET_PHASES;
        const NuerolynxStatusbarPacketPoint point =
            gui_nuerolynx_statusbar_packet_point(position);
        canvas_draw_dot(canvas, point.x, point.y);
    }
...
static void gui_redraw_status_bar(Gui* gui, bool need_attention) {
    ...
    if(momentum_settings.bar_background) {
        gui_draw_nuerolynx_statusbar_packets(gui->canvas);
    }
...
static bool gui_nuerolynx_statusbar_animation_active(Gui* gui) {
    if(!momentum_settings.bar_background) return false;

    gui_lock(gui);
    const bool lockdown = gui->lockdown && !gui->lockdown_inhibit;
    const bool active =
        !gui->direct_draw && gui->hide_statusbar_count == 0 &&
        !gui_view_port_find_enabled(gui->layers[GuiLayerFullscreen]) &&
        (!lockdown || momentum_settings.lockscreen_statusbar);
    gui_unlock(gui);
...
int32_t gui_srv(void* p) {
    ...
    while(1) {
        const uint32_t wait_timeout = gui_nuerolynx_statusbar_animation_active(gui) ?
                                          NUEROLYNX_STATUSBAR_PACKET_PERIOD_MS :
                                          FuriWaitForever;
        uint32_t flags =
            furi_thread_flags_wait(GUI_THREAD_FLAG_ALL, FuriFlagWaitAny, wait_timeout);
        if(flags == (uint32_t)FuriFlagErrorTimeout) {
            flags = GUI_THREAD_FLAG_DRAW;
        } else {

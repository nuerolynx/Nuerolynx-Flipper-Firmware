...
if not os.environ.get("DIST_SUFFIX"):
    ...
    try:
        # For tags, the distribution name is the release tag.
        release_tag = git("describe", "--tags", "--abbrev=0", "--exact-match")
        DIST_SUFFIX = (
            f"Nuerolynx-{release_tag}"
            if release_tag.lower().startswith("v")
            else release_tag
        )
    except Exception:
        # Otherwise identify the Nuerolynx branch and commit.
        branch_name = git("rev-parse", "--abbrev-ref", "HEAD").removeprefix("nlx-")
        commit_sha = git("rev-parse", "HEAD")[:8]
        DIST_SUFFIX = f"nuerolynx-{branch_name}-{commit_sha}"
    # Dist name is only for naming of output files
    DIST_SUFFIX = DIST_SUFFIX.replace("/", "-")
    # Firmware version is generated separately as "Nuerolynx <tag>".
